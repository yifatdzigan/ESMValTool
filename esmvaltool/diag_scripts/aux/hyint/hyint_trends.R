...
# setup parameters from r.interface
...
if (!all(is.element(field_names,names(nc$var)))) { 
  ...
  print(paste("HyInt_trends: missing variable in input indices file: ",missing))
  nc_close(nc)
...
for (var in field_names) {
  ...
  if (removedesert) { rfield<-rfield*retdes3D }
  if (maskSeaLand) { rfield<-apply.elevation.mask(rfield,relevation,seaLandElevation,reverse=reverse_maskSeaLand) }
  ...
  for (ireg in 1:nregions) { 
    ...
    if (lm_trend) { # linear regression
      ...
      if (length(which(!is.na(temp.tfield)))<2) { 
        ...
        rtrend[ireg,1:2]=lm_fit$coefficients # store trend coefficients (intercept and linear coef.) 
        rtrend_stat[ireg,]=lm_sum$coefficients[2,] # store trend coef., standard error, t value, Pr(>|t|) 
        print(lm_sum$coefficients[2,])
        if (length(trend_years) == 4) { # apply trend also to second time interval if required
          temp.tfield2<-tfield[ireg,rettimes2]
          if (length(which(!is.na(temp.tfield)))<2) {
            print("less than 2 points in second trend over selected region - skipping")
          } else {
            lm_fit2<-lm(temp.tfield2 ~ times[rettimes2])
            rtrend[ireg,3:4]=lm_fit2$coefficients # store 2nd interval trend coefficients
          }
        }
      }
    } 
...
stseries_list <- c(paste0(field_names,"_tseries"),
...
for (var in field_names) {
  for (itype in 1:length(var_type)) {
    ...
    metadata <- getmetadata.indices(var,infile)
    long_name <- metadata$long_name
    description <<- paste0(var_type_long[itype]," of ",metadata$long_name)
    units    <- metadata$units
    missval <- metadata$missing_value    
    #variable definitions
    var_ncdf=ncvar_def(svar,units,list(regiondim,timedim),missval,longname=long_name,prec="single",compression=1)
    if (itype > 2) { # trends
      var_ncdf=ncvar_def(svar,units,list(regiondim,coeffdim),missval,longname=long_name,prec="single",compression=1)
    }
    assign(paste0("var",svar),var_ncdf)
    assign(paste0("field",svar),rfield)
    assign(paste0(svar,"_description"),description)
  }
}

varregions=ncvar_def("regions","degrees",list(regiondim,boundarydim),-999,"region boundaries",prec="single",compression=1)
...
#namelist=c(stseries_list,strend_list,varregions,varregion_names,varregion_codes)
...
for (var in namelist){
  ...
  ncvar_put(ncfile, var, tmp.field, start = rep(1,ndims),  count = rep(-1,ndims))
  ncatt_put(ncfile, var, "description", get(paste0(var,"_description")))
}

# put additional attributes into dimension and data variables
#ncatt_put(ncfile,"time","axis","T")
ncatt_put(ncfile, "regions", "regionnames",  paste(fieldregion_names,collapse=" "))
ncatt_put(ncfile, "regions", "region_codes", paste(fieldregion_codes,collapse=" "))


# add global attributes
#print("Global attributes")
#print(str(nc_att_glob))
#print(nc_att_glob)
#ncatt_put(ncfile,0,"title",title$value)
#ncatt_put(ncfile,0,"institution",institution$value)
#ncatt_put(ncfile,0,"source",datasource$value)
#ncatt_put(ncfile,0,"references",references$value)
#history <- paste(author, date(), sep=", ")
#ncatt_put(ncfile,0,"history",history)
#ncatt_put(ncfile,0,"Conventions",Conventions$value)

# Get a summary of the created file:
ncfile

nc_close(ncfile)
print(paste(diag_base,": netCDF file saved"))
