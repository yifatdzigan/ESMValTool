...
outfile<-getfilename.indices(save_dir,diag_base,rgrid,model_idx,season)
...
if (!file.exists(inregfile)) { stop("HyInt: missing regridded input file. Run HyInt pre-processing.") }
...
# file opening
pr_list=ncdf.opener.time(infile,"pr",tmonths=timeseason,tyears=years,rotate="full")
...
if (external_r95[1]==F) {
  # pr_wet_temp<-pr
  # ret_wet_temp<-(pr>=1)         # select wet days
  # pr_wet_temp[ret_wet_temp]<-NA # mask dry days
  ...
  if (external_r95[1]=="HIST") { # if required, use HyInt file from historical period 
    ...
    external_r95<-getfilename.indices(hist_dir,diag_base,rgrid,model_idx,season,hist=T,hist_years=norm_years)
  }
...
for (iyear in 1:nyear) {
  ...
  if (length(pr_year[1,1,])==366) { r95_thresh_year <- r95_threshold366 }
  ...
  ret_below_r95<-(pr_year<r95_thresh_year) # Rainy days when pr < reference 95% quantile
  ...
  pr_year_r95<-pr_year    ; pr_year_r95[ret_below_r95]<-NA # actual precipitation but with NA on days with pr < reference 95% quantile
  ...
  # Heavy precipitation Index (R95) Percent of total precipitation above the 95% percentile of the reference distribution;
  r95_year<-apply(pr_year_r95,c(1,2),sum,na.rm=T)/apply(pr_year,c(1,2),sum,na.rm=T)*100.
...
# NOTE: take care of normalization by 0: when the normalizing function is 0 (e.g. short dataset in reference period), the resulting normalized index will be NA.

if (external_norm[1]==F) { # calculate normalization function 
  ret_years<-which(years >= norm_years[1] & years <= norm_years[2])
  if (length(ret_years)==0) {stop("HyInt: no data over selected normalization period, unable to normalize")}
  ...
  if (external_norm[1]=="HIST") { # if required, use HyInt file from historical period 
    hist_dir=file.path(work_dir,exp,paste0(norm_years[1],"_",norm_years[2]),season)
    external_norm<-getfilename.indices(hist_dir,diag_base,rgrid,model_idx,season,hist=T,hist_years=norm_years)
  }
...
if (external_norm[1]==F) { # calculate or load hyint_mean from file to be consistent with other indices 
  ret_years<-which(years >= norm_years[1] & years <= norm_years[2])
...
# HyInt list
hyint_list<-list(pry=pry,dsl=dsl,wsl=wsl,int=int,pa=pa,r95=r95,hyint=hyint,
                 pry_mean=pry_mean,dsl_mean=dsl_mean,wsl_mean=wsl_mean,int_mean=int_mean,pa_mean=pa_mean,r95_mean=r95_mean,hyint_mean=hyint_mean,
                 pry_mean_sd=pry_mean_sd,dsl_mean_sd=dsl_mean_sd,wsl_mean_sd=wsl_mean_sd,int_mean_sd=int_mean_sd,
                 pa_mean_sd=pa_mean_sd,r95_mean_sd=r95_mean_sd,hyint_mean_sd=hyint_mean_sd,
                 pry_norm=pry_norm,dsl_norm=dsl_norm,wsl_norm=wsl_norm,int_norm=int_norm,pa_norm=pa_norm,r95_norm=r95_norm, r95_threshold=r95_threshold)

print(paste(diag_base,": calculation done. Returning mean precipitation, sdii, dsl, wsl, pa, r95 (absolute and normalized values) and hyint indices"))
...
field_list<-c("pry","dsl","wsl","int","pa","r95","hyint","pry_mean","dsl_mean","wsl_mean","int_mean","pa_mean","r95_mean","hyint_mean",
...
for (var in field_list) {
  ...
  metadata <- setmetadata.indices(var)
  longvar <- metadata$longvar
  unit    <- metadata$unit
  #variable definitions
  var_ncdf=ncvar_def(var,unit,list(x,y,t),-999,longname=longvar,prec="single",compression=1)     
  if ((var=="pry_mean")|(var=="int_mean")|(var=="dsl_mean")|(var=="wsl_mean")|(var=="pa_mean")|(var=="r95_mean")|(var=="hyint_mean")|
      (var=="pry_mean_sd")|(var=="int_mean_sd")|(var=="dsl_mean_sd")|(var=="wsl_mean_sd")|(var=="pa_mean_sd")|(var=="r95_mean_sd")|
      (var=="hyint_mean_sd")|(var=="r95_threshold")) { 
