...
#for (iregion in c(1:nregions)) {
  ...
  # Startup graphics for boxplots of trend coefficients 
  if (plot_type == 7) {
      ...
      if (plot_type >= 5 & iyear == 1) {
        ...
        for (ireg in 1:nregions) { 
          ...
          if (lm_trend) { # linear regression model 
            ...
            if (add_trend) { lines(times[rettimes],predict(lm_fit),col=ireg,lwd=1.5) } 
            if (length(trend_years) == 4) { # apply trend also to second time interval if required
              ...
              if (add_trend) { lines(times[rettimes2],predict(lm_fit2),col=ireg,lwd=1.5) } 
            }
          } 
        }
        ...
        if (plot_type == 7) { # plot trend coefficients for different regions, one panel per field
          ...
          xregions=1:nregions
          plot(xregions,trend_exp[,2],type="n",pch=22,axes=F,xlab="regions",ylab=paste0("Avg trend (over ",length(rettimes)," years)"),
                  ylim=ylim, main=(paste(title_unit_m[ifield,3]," trends")))          
          ...
          points(xregions, trend_exp[,2], pch=22, col="grey40", bg="white",cex=3)
          # add filled points for significant (95% level)
          retsig90=which(trend_exp_stat[,4]<0.1)
          if (!is.na(retsig90[1])) { points(xregions[retsig90], trend_exp[retsig90,2], pch=22, col="grey70", bg="grey40",cex=3) }
          retsig95=which(trend_exp_stat[,4]<0.05)
          if (!is.na(retsig95[1])) { points(xregions[retsig95], trend_exp[retsig95,2], pch=22, col="grey40", bg="grey40",cex=3) }
          axis(1,labels=region_names[selregions],at=xregions)
          axis(2)                  
       } 
      }
